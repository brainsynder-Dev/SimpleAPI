...
public class SpigotPluginHandler implements Listener {
    ...
    private static class MetricsLite {
        ...
        public boolean start() {
            ...
            try {
                ...
                if (debug) {
                    Bukkit.getLogger().log(Level.INFO, '[' + plugin.getDescription().getName() + "] " + e.getMessage());
                }
            }
        ...
        public boolean isOptOut() {
            synchronized (optOutLock) {
                try {
                    ...
                    if (debug) {
                        Bukkit.getLogger().log(Level.INFO, '[' + plugin.getDescription().getName() + "] " + ex.getMessage());
                    }
                    return true;
                }
        ...
        public File getConfigFile() {
            ...
            return new File(pluginsFolder + "/PluginMetrics", "metrics.yml");
        }
        ...
        public void postPlugin(boolean isPing) throws IOException {
            if (isOptOut())
                return;
            ...
            if (debug) {
                System.out.println('[' + plugin.getDescription().getName() + "] Prepared request for " + pluginName + " uncompressed=" + uncompressed.length + " compressed=" + compressed.length);
            }
